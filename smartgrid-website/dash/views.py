...
def dash_statistics_mean_solution(request):
    ...
    if len(Reading.objects.filter(**{form_1_time_period_selection: form_2_time_period_selection})) == 0:
        messages.warning(request,
                         f'There are no Readings with {form_1_time_period_selection} {form_2_time_period_selection} in the database.')
        return redirect('/dash/dash_statistics_mean_1/')
    ...
    if (len(reading_search_list) == 0):
        messages.warning(request,
                         f"There are no objects that match your search criteria. Please change the search paremeters and try again.")
        return redirect('/dash/dash_statistics_mean_1/')
